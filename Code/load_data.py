...
def load_dir(path) :
    ...
    for fn in pdf_files : 
        ...
        try :
            ...
            lo_texts = lo_texts.append({'doc_id' : fn.split('/')[-1],
                                         'text' : txt}, ignore_index = True )
        except :
...
def load_data(path, include_subdir = True, ignore_prefix = '_') :
    df = pd.DataFrame()
    ...
    for dirc in tqdm(lo_dirs) :
            ...
            k = len(ignore_prefix)
            if len(ignore_prefix)==0 or (author_name[:k] != ignore_prefix) :  #ignore symbol
